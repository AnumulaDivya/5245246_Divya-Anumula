 ...
 *
 */
int* climbingLeaderboard(int rankedCount, int* ranked, int playerCount, int* player, int* resultCount) {
      int* unique = malloc(rankedCount * sizeof(int));
    int uniqueCount = 0;
    ...
    // Step 2: Allocate results
    int* result = malloc(playerCount * sizeof(int));
    *resultCount = playerCount;

    // Step 3: Walk leaderboard backwards for efficiency
    int idx = uniqueCount - 1;  // start from lowest unique score
    for (int i = 0; i < playerCount; i++) {
        int score = player[i];
        while (idx >= 0 && score >= unique[idx]) {
            idx--; // move left if score is high enough
        }

        if (idx == -1) {
            // player beat all scores
            result[i] = 1;
        } else {
            result[i] = idx + 2;  // +1 for rank, +1 since idx points to lower score
        }
    }
